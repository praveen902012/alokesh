...
export default function GreenTheme() {
  ...
  return (
    <div className="theme-green bg-background min-h-screen font-body-md text-on-background antialiased transition-colors duration-300">
      {/* TopNavBar */}
      <nav className="sticky top-0 z-50 flex justify-between items-center h-20 px-margin-desktop w-full max-w-container-max mx-auto bg-surface-container-lowest shadow-md">
        <div className="flex items-center gap-gutter">
          ...
          <div className="hidden md:flex gap-8 items-center h-full">
            ...
            <a className="text-on-surface-variant hover:text-primary transition-colors font-body-md text-body-md cursor-pointer active-scale" href="#">Rent</a>
            <a className="text-on-surface-variant hover:text-primary transition-colors font-body-md text-body-md cursor-pointer active-scale" href="#">Sell</a>
            <a className="text-on-surface-variant hover:text-primary transition-colors font-body-md text-body-md cursor-pointer active-scale" href="#">Projects</a>
            <a className="text-on-surface-variant hover:text-primary transition-colors font-body-md text-body-md cursor-pointer active-scale" href="#">Dealers</a>
          </div>
        </div>
        <div className="flex items-center gap-stack-md">
          <button className="bg-primary-container text-on-primary px-6 py-2.5 rounded-lg font-label-md text-label-md hover:opacity-90 transition-opacity shadow-md active-scale">Post Property</button>
      ...
      <header className="relative w-full h-[540px] flex items-center justify-center px-margin-desktop mb-stack-lg">
        ...
        <div className="relative z-10 w-full max-w-4xl text-center">
          ...
          {/* Search Tabs & Bar */}
          <div className="bg-surface-container-lowest rounded-xl shadow-2xl p-4 md:p-6 mx-auto">
            <div className="flex gap-stack-md mb-6 border-b border-surface-container">
              {['Buy', 'Rent', 'Projects'].map(tab => (
                <button
                  key={tab}
                  onClick={() => setSearchTab(tab)}
                  className={`pb-3 px-4 transition-all ${searchTab === tab ? 'text-primary border-b-2 border-primary font-bold' : 'text-on-surface-variant hover:text-primary'}`}
                >
                  {tab}
                </button>
              ))}
            ...
            <div className="flex flex-col md:flex-row gap-4 items-center">
              <div className="flex-1 w-full flex items-center bg-surface-container-low rounded-lg px-4 py-3 border border-outline-variant">
                ...
                <select 
                  ...
                  className="bg-transparent border-none focus:ring-0 text-on-surface font-body-md w-32 cursor-pointer outline-none"
                >
              ...
              <button className="w-full md:w-auto bg-primary-container text-on-primary px-10 py-4 rounded-lg font-headline-md flex items-center justify-center gap-2 hover:opacity-90 transition-all active:scale-95 shadow-md">
                Search
              </button>
            </div>
          </div>
      ...
      <main className="max-w-container-max mx-auto px-margin-desktop py-stack-lg">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-gutter">
            {[
              ...
              <div 
                ...
                className={`border p-gutter rounded-xl flex flex-col items-center gap-stack-sm hover:shadow-lg transition-all cursor-pointer group ${filterType === cat.name ? 'border-primary bg-surface-container-low shadow-md' : 'bg-surface-container-lowest border-outline-variant'}`}
              >
                <div className={`w-16 h-16 rounded-full flex items-center justify-center transition-colors ${filterType === cat.name ? 'bg-primary-container text-on-primary' : 'bg-surface-container-low text-primary group-hover:bg-primary-container group-hover:text-on-primary'}`}>
        ...
        <section className="mb-stack-lg">
          <div className="flex justify-between items-end mb-stack-md">
            <div>
              ...
              <p className="text-on-surface-variant font-body-md">Handpicked premium listings in {location}</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-gutter">
            {filteredProperties.length > 0 ? (
              filteredProperties.map(prop => (
                <article key={prop.id} className="bg-surface-container-lowest rounded-xl border border-outline-variant overflow-hidden hover:shadow-2xl transition-all group cursor-pointer">
                  ...
                  <div className="p-stack-md">
                    ...
                    <div className="flex justify-between border-y border-surface-container py-3 mb-4">
                      <div className="text-center">
                        <p className="text-on-surface-variant text-label-sm">Config</p>
                        <p className="font-label-md">{prop.config}</p>
                      </div>
                      <div className="text-center">
                        <p className="text-on-surface-variant text-label-sm">Area</p>
                        <p className="font-label-md">{prop.area}</p>
                      </div>
                      <div className="text-center">
                        <p className="text-on-surface-variant text-label-sm">Status</p>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button className="border border-primary text-primary px-4 py-1.5 rounded-lg text-label-sm font-bold hover:bg-primary hover:text-on-primary transition-all">Details</button>
                    </div>
                  </div>
                </article>
              ))
        ...
        {/* Verified Property Experts */}
        <section className="mb-stack-lg">
          <h3 className="font-headline-lg text-headline-lg mb-stack-md">Verified Property Experts</h3>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-gutter">
            {[
              ...
              <div key={idx} className="bg-surface-container-lowest p-gutter border border-outline-variant rounded-xl flex flex-col items-center text-center hover:shadow-lg transition-all">
                ...
                <p className="text-on-surface-variant text-body-sm mb-4">{agent.agency}</p>
        ...
        <section className="mb-stack-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-gutter">
            {[
              ...
              <div key={idx} className="flex items-center gap-4 p-4 border border-outline-variant rounded-xl hover:bg-surface-container-low transition-colors cursor-pointer bg-surface-container-lowest">
                <div className="w-16 h-16 bg-surface-container-lowest border border-surface-container flex items-center justify-center p-2 rounded-lg">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-on-surface-variant text-body-sm">{builder.active} Projects • {builder.ongoing} Ongoing</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <footer className="w-full py-stack-lg px-margin-desktop max-w-container-max mx-auto border-t border-outline-variant grid grid-cols-1 md:grid-cols-4 gap-gutter bg-surface-container-lowest">
        <div className="col-span-1 text-left">
          ...
          <p className="text-on-surface-variant text-body-sm mb-stack-md">Simplifying your property journey with data-driven insights and verified listings since 2012.</p>
        ...
        <div className="text-left">
          ...
          <ul className="flex flex-col gap-2">
            <li><a className="text-on-surface-variant hover:text-primary transition-all font-body-sm" href="#">About Us</a></li>
            <li><a className="text-on-surface-variant hover:text-primary transition-all font-body-sm" href="#">Careers</a></li>
            <li><a className="text-on-surface-variant hover:text-primary transition-all font-body-sm" href="#">Privacy Policy</a></li>
            <li><a className="text-on-surface-variant hover:text-primary transition-all font-body-sm" href="#">Terms of Service</a></li>
          </ul>
        </div>
        <div className="text-left">
          <h6 className="font-label-md text-primary mb-stack-md">Services</h6>
          <ul className="flex flex-col gap-2">
            <li><a className="text-on-surface-variant hover:text-primary transition-all font-body-sm" href="#">Property Management</a></li>
            <li><a className="text-on-surface-variant hover:text-primary transition-all font-body-sm" href="#">Legal Consultation</a></li>
            <li><a className="text-on-surface-variant hover:text-primary transition-all font-body-sm" href="#">Home Loans</a></li>
            <li><a className="text-on-surface-variant hover:text-primary transition-all font-body-sm" href="#">Interior Design</a></li>
          </ul>
        </div>
        <div className="text-left">
          <h6 className="font-label-md text-primary mb-stack-md">Newsletter</h6>
          <p className="text-on-surface-variant text-body-sm mb-4">Stay updated with market trends.</p>
          <div className="flex gap-2">
            ...
            <button className="bg-primary-container text-on-primary p-2 rounded-lg flex items-center justify-center hover:opacity-90 transition-opacity"><span className="material-symbols-outlined">send</span></button>
          </div>
        </div>
        <div className="col-span-full border-t border-surface-container mt-stack-lg pt-stack-md flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-on-surface-variant text-body-sm">© 2024 StatLynk Real Estate Solutions. All rights reserved.</p>
          <div className="flex gap-stack-md">
